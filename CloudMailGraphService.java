...
@Service
public class CloudMailGraphService {
    ...
    private ProxyOptions buildProxyOptions() throws CryptoUtilException {
        ...
        String proxyHost = System.getProperty("https.graphProxyHost");
        String proxyPort = System.getProperty("https.graphProxyPort");
        String proxyUser = PROXY_USER.getValue();
        String proxy_password = PROXY_PASSWORD.getValue();
        if (StringUtils.isBlank(proxy_password)) {
            proxy_password = System.getenv().get("EDOC_PROXY_PASSWORD");
            if (StringUtils.isBlank(proxy_password)) {
            ...
            LOGGER.info("proxy_password length: {}", StringUtils.isNotBlank(proxy_password) ? proxy_password.length() : "EMPTY");
        }

        LOGGER.info("Graph proxy config - Host: {}, Port: {}, User: {}", proxyHost, proxyPort, proxyUser);

        // Check if all required proxy settings are available
        if (StringUtils.isBlank(proxyHost) || StringUtils.isBlank(proxyPort) ||
                StringUtils.isBlank(proxyUser) || StringUtils.isBlank(proxy_password)) {
            LOGGER.info("Graph proxy not fully configured, using direct connection");
            return null;
        }
        ...
        try {
            ...
            proxyOptions.setCredentials(proxyUser, proxy_password);
            LOGGER.info("Graph proxy configured successfully with authentication");
            return proxyOptions;
    ...
    public void saveMailToOutbox(String senderEmail, List<String> domainPasswordPair, Mail mail, byte[] attachment, String fileName) throws CryptoUtilException {
        ...
        SingleValueLegacyExtendedProperty flagProp = new SingleValueLegacyExtendedProperty();
        flagProp.setId("Integer 0x0E07");
        flagProp.setValue("0");
        List<SingleValueLegacyExtendedProperty> extendedProperties = new ArrayList<>();
        extendedProperties.add(flagProp);
        messageToSave.setSingleValueExtendedProperties(extendedProperties);
    ...
    private Message getGraphRequestMessage(Mail mail, byte[] attachment, String fileName) {
        ...
        Recipient senderRecipent = new Recipient();
        EmailAddress senderEmailAddress  = new EmailAddress();
        senderEmailAddress.setAddress(resolveSenderEmail(mail));
        senderRecipent.setEmailAddress(senderEmailAddress);
        senderEmailAddress.setName(resolveSenderEmail(mail));
        message.setFrom(senderRecipent);
        message.setSender(senderRecipent);
